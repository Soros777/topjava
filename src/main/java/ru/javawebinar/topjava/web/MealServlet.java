...
public class MealServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        switch (action == null ? "all" : action) {
            case "filter":
                LocalDate[] boundedDate = ServletUtil.getBoundedDate(request.getParameterMap());
                LocalTime[] boundedTime = ServletUtil.getBoundedTime(request.getParameterMap());
                Collection<Meal> meals = repository.getFilteredByDate(SecurityUtil.authUserId(), boundedDate[0], boundedDate[1]);
                request.setAttribute("meals",
                        MealsUtil.getFilteredTos(meals, SecurityUtil.authUserCaloriesPerDay(), boundedTime[0], boundedTime[1]));
